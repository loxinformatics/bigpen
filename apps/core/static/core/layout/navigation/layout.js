...
function updateActiveStates() {
  ...
  document.querySelectorAll('.navmenu .dropdown.active:not(.open-by-default)').forEach(dropdown => {
    dropdown.classList.remove('active');
  });
  document.querySelectorAll('.navmenu ul.dropdown-active:not(.open-by-default ul)').forEach(ul => {
  ...
  document.querySelectorAll('.navmenu a[href]').forEach(link => {
    ...
    const isExactMatch = linkPath === currentPathname && linkHash === currentHashname;
    const isPathMatch = linkPath === currentPathname && linkPath !== '';
    const isHashMatch = currentHashname && linkHash && linkHash === currentHashname;
    ...
    if (isExactMatch || isPathMatch || isHashMatch) {
      ...
      if (parentDropdown) {
        ...
        const parentLink = parentDropdown.querySelector('a.navlink, a.toggle-dropdown-link');
        if (parentLink) {
...
document.querySelectorAll('#aside #navmenu a').forEach(navmenu => {
  navmenu.addEventListener('click', () => {
    ...
    if (!navmenu.classList.contains('toggle-dropdown-link') && document.querySelector('.aside-show')) {
      asideToggle();
    }
  });
});
 ...
 */
document.querySelectorAll('.aside .navmenu .toggle-dropdown-link').forEach(link => {
...
document.querySelectorAll('.aside .navmenu .dropdown.open-by-default').forEach(item => {
 ...
 * Mobile nav toggle
 */
const headerMobileNavToggleBtn = document.querySelector('#header .mobile-nav-toggle');
