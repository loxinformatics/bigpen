from core.models import CoreProvider
...
def robots_txt(request):
    extra_context = {}
    return render(request, "core/robots.txt", extra_context, content_type="text/plain")
...
def home(request):
    ...
    if request.method == "POST":
        ...
        if filled_form.is_valid():
            ...
            recipient_email = (
                CoreProvider.objects.first().primary_email
                if CoreProvider._meta.db_table in connection.introspection.table_names()
                else None,
            )
            ...
            text_content = render_to_string("core/mail/mail_us.txt", email_context)
            html_content = render_to_string("core/mail/mail_us.html", email_context)
    ...
    return render(request, "core/homepage.html", extra_context)
